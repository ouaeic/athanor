...
const isBenchmarked = (ranked: RankedModel): boolean =>
  ranked.breakdown.quality.source === 'benchmark' || ranked.breakdown.quality.source === 'overall';

const routeLabel = (route: PrivacyRoute): string =>
  route === 'provider_zdr' ? 'the zero-retention route' : 'the external route';

const describeCeiling = (request: ModelRequest): string => {
  const input = request.maxInputUsdPerMillionTokens;
  const output = request.maxOutputUsdPerMillionTokens;
  if (typeof input === 'number' && typeof output === 'number')
    return `${formatUsd(input)}/${formatUsd(output)}`;
  if (typeof input === 'number') return `${formatUsd(input)} per million input`;
  return `${formatUsd(output ?? 0)} per million output`;
};

const describePrice = (model: RoutableModel, request: ModelRequest): string => {
  const { input, output } = pricesAtPromptSize(model, request.minContextTokens);
  if (input === null && output === null) return 'an unpublished price';
  if (input !== null && output !== null) return `${formatUsd(input)}/${formatUsd(output)}`;
  return formatUsd(input ?? output ?? 0);
};

const cheapestFirst = (left: RoutableModel, right: RoutableModel): number => {
  const leftPrice = blendedPricePerMillionTokens(left);
  const rightPrice = blendedPricePerMillionTokens(right);
...
export interface PriceCeilingReport {
  ...
  readonly bestUnderCeiling: RankedModel | null;
  readonly bestWithoutCeiling: RankedModel | null;
  /** The three lines a settings screen renders, computed live rather than asserted. */
  readonly lines: readonly string[];
}

const scoreLine = (ranked: RankedModel, request: ModelRequest, lead: string): string => {
  const profile = taskProfile(request.taskKind ?? 'general');
...
/**
 * A ceiling read from a sentence, or the reason it could not be read. Never a guess: a spending
 * control that half-understands what the owner typed is worse than one that says it did not.
 */
export type SpendCeilingReading =
  | { readonly ok: true; readonly ceiling: SpendCeiling | null; readonly sentence: string }
  | { readonly ok: false; readonly problem: string };

/** The phrases that mean "no ceiling", so the owner who does not want one can say so and be done. */
const NO_CEILING = /^(?:none|no|off|any|no ceiling|no limit|no cap|unlimited|unlimited spend)$/;

/**
 * Which rate a figure is. The unambiguous words are taken anywhere; the bare `in` and `out` only
 * where a rate is plainly what is meant, so "under $2 in total" is refused rather than read as an
 * input rate of two dollars.
 */
const ROLE = String.raw`input|prompt|output|completion|in(?=\s*(?:$|[,;./&]|and\b|per\b))|out(?=\s*(?:$|[,;./&]|and\b|per\b))`;
/**
 * A thousands separator is refused rather than truncated: reading "$2,000" as two dollars would set
 * a ceiling a thousandfold tighter than the one that was typed, silently. Only a separator that is
 * actually inside a number is refused - the lookahead needs a digit behind the comma - so "$2,
 * output $10" still reads, and the lookbehind stops the "000" being picked up as a figure of its own.
 */
const AMOUNT = String.raw`\$?\s*(?<![\d,.])(\d+(?:\.\d+)?)(?![.,]?\d)`;
const PER_MILLION = String.raw`(?:\/\s*m(?:illion)?\b|per\s+million(?:\s+tokens?)?)`;

const amountThenRole = new RegExp(
  `${AMOUNT}\\s*(?:usd|dollars?)?\\s*${PER_MILLION}?\\s*(?:for\\s+|of\\s+|on\\s+|the\\s+)*(${ROLE})`,
  'gi'
);
const roleThenAmount = new RegExp(
  `(${ROLE})\\b(?:\\s+tokens?)?\\s*(?:rate\\s*)?(?:is|at|of|=|:)?\\s*${AMOUNT}`,
  'gi'
);

const isOutputRole = (role: string): boolean =>
  role === 'output' || role === 'completion' || role === 'out';
