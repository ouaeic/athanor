...
import { registerComputationRoutes } from './computation.js';
import type { RouteContext } from '../http/server-context.js';
const session = 'kernel-11111111-1111-4111-8111-111111111111';
...
describe('owner computation controls', () => {
    ...
    try {
      expect(
        (
          await app.inject({
            method: 'GET',
            url: '/v1/workspaces/owned/computation',
      ...
      expect(runner.request).toHaveBeenLastCalledWith({
        ...
        role: 'user',
        scopes: ['files.read'],
        path: '/v1/workspaces/owned/computation',
        method: 'GET'
      });
      expect(
        (
          await app.inject({
            method: 'POST',
            url: `/v1/workspaces/owned/computation/${session}/control`,
            headers: { 'x-test-user': 'owner' },
            payload: { action: 'stop' }
          })
        ).statusCode
    ...
    try {
      expect(
        (
          await app.inject({
            method: 'POST',
            url: `/v1/workspaces/owned/computation/${id}/control`,
