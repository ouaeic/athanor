...
export function App() {
   ...
   *
   * Not `taskIsActive`: this is every conversation on the box, because the reason to look at the
   * corner of the screen is to find out whether the machine is doing anything at all, not whether
   * the conversation currently open is.
   */
  /*
   * Out means we looked and the fire is not lit. It does not mean we have not looked yet.
   *
   * This derivation runs above the auth gate, so on the splash and the sign-in pair there is no
   * bootstrap and no workspace - and the first version read that as `cold`, which put a grey flame
   * at a third height on the first screen a new owner ever sees. That is the interface asserting
   * something it does not know, which is the fault this whole design exists to remove, and it
   * asserted it about the one thing the owner has not been able to check yet.
   *
   * So the fire rests until the box has answered. Banked is the honest state for "not yet told":
   * the furnace this is named after is never out, it is only ever low.
   */
  const fireState: FireState =
    auth !== 'ready' || !data
      ...
      : offline || !workspace || workspace.status === 'failed' || workspace.status === 'hibernated'
        ? 'cold'
        : approvals.length > 0 || data.tasks.some((item) => item.status === 'awaiting_user')
          ? 'calling'
          : data.tasks.some((item) => isLiveTask(item) && taskIsGenerating(item.status))
            ? 'drawing'
            : 'banked';
  useEffect(() => startFire(), []);
  useEffect(() => setFire(fireState), [fireState]);
